...
**6c 後續診斷（2026-08-23，`strategy/benchmark_concentration.py`，
`reports/benchmark_concentration_analysis.md`）**：針對「策略顯著跑輸市值加權 TAIEX」
這個發現，額外診斷這是選股能力問題還是大盤被台積電這種巨型股撐起來的集中度效應。用
「移除法」真實計算：台積電貢獻代理市值加權指數總報酬至少 18.9%（代理指數用成交量代理
市值，方法論限制導致這個數字可能低估，已誠實揭露）。策略對排除台積電的代理 TAIEX
（p=0.5074）與等權重代理 TAIEX（p=0.4192）都**不再顯著跑輸**——代表原本的負面結果主要
是大盤集中度效應，不是選股能力本身特別差。**但策略對真實市值加權 TAIEX 顯著跑輸這個
原始發現完全沒有改變**（p=0.0272，不變），Task 9 寫 README 時兩個發現都要並列呈現，
不能只挑對策略有利的半句話。新增 `tests/test_benchmark_concentration.py`（5 項），
pytest 全部 72/72 通過。完整決策記錄見 `docs/DECISIONS.md`「Task 6c 後續診斷」那筆。
...
4. ~~**requirements1.txt 缺依賴**~~ **已修復**——`matplotlib`、`pytest` 於 Task 2 期間補進
   ...
   `tejapi` 於 Task 7a 補進（`data_pipeline/survivorship.py` 的 TEJ 可用性檢查要用，
   真實測試後發現金鑰已過期，改走 FinMind fallback，見 Task 7 細節）。
...
11. ~~**`mom_120`／`div_yld` 兩個舊版遺留因子從未經過正式 IC 排查，卻一直真的在參與選股**~~
    **已修復（2026-09-03）**——Task 8 驗收前排查發現，見 `docs/DECISIONS.md`「Task 8
    驗收前疑點排查」。`mom_120`（IC 0.0212，跟主動量因子排名相關係數 0.692，冗餘）
    已移出複合；`div_yld`（IC 0.0424，五因子裡最高）已正式納入，`default_weights`
    依真實 IC 重新計算。

12. **（Task 8 驗收前發現，已釐清非問題）真實回測總報酬（129.6%）跟 Task 6 報告
    （92.1%）數字不一致**——完整拆解確認幾乎全部差異來自資料回測區間延伸約 4.3 個月
    （不是先前以為的一週），期間 TAIEX 真實大漲 +25.9%，複利計算精確對得上；因子修正
    （見上方第 11 項）只貢獻其中一小部分。Task 6 原始報告本身正確，只是時間點較早，
    不需要重新執行，詳見 `docs/DECISIONS.md`。

13. **（Task 8 驗收前發現，未修復，僅誠實記錄限制）ML 崩盤預警模型（`regime/ml_alert.py`）
    13 年 pooled 樣本外 AUC = 0.493，判別力跟隨機猜測沒有顯著差異**——根因是崩盤標籤
    稀少且高度自相關（多數年份只有 1-6 次獨立事件），不是程式 bug。完整診斷見
    `reports/ml_alert_reliability_diagnosis.md`。**刻意沒有**修改 `regime_engine.py`
    的分類公式（會讓 Task 3-6 已定案的數字全部過時，代價超過這次排查範圍），只在文件
    誠實記錄這個限制，供你之後決定是否要調整三方投票設計。
